...
async function main() {
  ...
  // --- 1. Escuchar NFTMinted ---
  nftContract.on("NFTMinted", async (buyer, tokenId, nftType, event) => {
  ...
  // --- 2. Escuchar Voted ---
  govContract.on("Voted", async (voter, proposalId, weight, event) => {
    const txHash = event.log.transactionHash;
  ...
  // --- 3. Escuchar DonationReceived ---
  crowdfundContract.on("DonationReceived", async (campaignId, donor, amount, event) => {
    const txHash = event.log.transactionHash;
  ...
  console.log("🚀 Listeners active and running in background.");
